...
class VectorPlotter:
    def __init__(self, ax_limits=None, colors=None, end_color='r', end_markersize=3, connecting_style='-'):
    ...
    def plot_vectors(self, vectors):
        ...
        for length, angle in vectors:
            ...
            circle = plt.Circle(
                current_endpoint, length, color='b', fill=False, linestyle='dotted', linewidth=0.5
            )
    ...
    def draw_all_endpoints(self):
        ...
        self.ax.scatter(
            endpoints_np[:, 0], endpoints_np[:, 1], self.end_markersize, color=self.end_color
        )
    ...
    def draw_connected_dots(self):
        endpoints_np = np.array(self.endpoints)
        self.ax.plot(endpoints_np[:,0], endpoints_np[:,1], linestyle=self.connecting_style, color=self.end_color)
